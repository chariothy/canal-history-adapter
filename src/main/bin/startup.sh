...
if [ -f $base/bin/canal-history-app.pid ] ; then
	echo "found canal-history-app.pid , Please run stop.sh first ,then startup.sh" 2>&2
    exit 1
fi
...
CANAL_OPTS="-DappName=canal-history-app -Dlogback.configurationFile=$logback_configurationFile"
...
then 
	...
	if [ $client_mode == "Cluster" ] ; then 
		$JAVA $JAVA_OPTS $JAVA_DEBUG_OPT $CANAL_OPTS -classpath .:$CLASSPATH net.chariothy.db.CanalHistoryaApp 1>>$base/bin/nohup.out 2>&1 &
	else 
		$JAVA $JAVA_OPTS $JAVA_DEBUG_OPT $CANAL_OPTS -classpath .:$CLASSPATH net.chariothy.db.CanalHistoryApp 1>>$base/bin/nohup.out 2>&1 &
	fi
	
	echo $! > $base/bin/canal-history-app.pid
