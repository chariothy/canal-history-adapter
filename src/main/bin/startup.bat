...
set CANAL_OPTS=-DappName=canal-history-app -Dlogback.configurationFile="%logback_configurationFile%"

set JAVA_OPTS= %JAVA_MEM_OPTS% %JAVA_OPTS_EXT% %JAVA_DEBUG_OPT% %CANAL_OPTS%

if "%client_mode%" == "Cluster" (
	java %JAVA_OPTS% -classpath "%CLASSPATH%" net.chariothy.db.CanalHistoryApp
) else (
	java %JAVA_OPTS% -classpath "%CLASSPATH%" net.chariothy.db.CanalHistoryApp
)
